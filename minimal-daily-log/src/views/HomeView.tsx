...
import { MessageSquare, Moon, Image as ImageIcon, X, BarChart2, CheckCircle2, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function HomeView() {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      updateRecord({ imageUrl: reader.result as string });
    };
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="pb-24">
      ...
      <div className="mb-8">
        <label className="block w-full aspect-[2/1] bg-white/40 backdrop-blur-md border border-white/60 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.02)] rounded-[24px] flex flex-col items-center justify-center text-stone-400 cursor-pointer overflow-hidden transition-all hover:bg-white/50 relative group">
          {todayRecord.imageUrl ? (
            <>
              <img src={todayRecord.imageUrl} alt="today" className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                 <span className="text-white text-xs font-bold drop-shadow-md">사진 변경</span>
              </div>
              <button 
                type="button"
                onClick={(e) => { e.preventDefault(); updateRecord({ imageUrl: undefined }); }}
                className="absolute top-3 right-3 bg-black/40 p-1.5 rounded-full text-white backdrop-blur-md hover:bg-black/60 focus:outline-none"
              >
                <X size={16} />
              </button>
            </>
             ...
             <div className="flex flex-col items-center gap-2">
                <ImageIcon size={28} className="text-stone-300 mb-1" />
                <span className="text-[13px] font-bold text-stone-400">오늘의 순간 기록하기</span>
             </div>
          )}
